...
table_file = "2024_APOE_ATN_biomarkers/Results/20250328_table_s9.xlsx"

# Read in data from xlsx file 
df <- read_excel(dat_file)

df <- df %>% 
  mutate(count = as.numeric(proportion)*2) %>% 
  select(biomarker, allele, ancestry, count, est, paper_pval) %>% 
  pivot_wider(names_from = allele, values_from = c("est", "paper_pval")) %>% 
  select(biomarker, ancestry, count, est_E2, paper_pval_E2, est_E4, paper_pval_E4) %>% 
...
row2 <- c("Biomarker", "Genetic Ancestry", "Count", "est[CI]", "p-value", "est[CI]", "p-value")
