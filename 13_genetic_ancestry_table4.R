...
table4_file = "2024_APOE_ATN_biomarkers/Results/20250123_table4.xlsx"
ancestry_associations_file = "2024_APOE_ATN_biomarkers/Data/20250123_additive_partially_adjusted_associations_by_genetic_ancestry.xlsx"
...
tbl4 <- data.frame("ATN biomarker" = c("Aꞵ42/40", "", "pTau-181", "", "NfL", "", "GFAP", ""), 
...
for(ancestry in genetic_ancestry_groups) { 
  ...
  ancestry_interaction_df <- ancestry_df %>% 
    ...
    select(c("est[CI]", "p-value"))
  tbl4 <- cbind(tbl4, ancestry_interaction_df, ancestry_allele_df)
} 
...
tbl4 <- rbind(row1, row2, row3, tbl4)

write.xlsx(tbl4, file = table4_file, overwrite = TRUE, colNames=FALSE)  
