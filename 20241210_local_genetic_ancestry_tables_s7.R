# Creating df for Table 5: Associations between ATN biomarkers and APOE allele by local genetic ancestry
...
local_ancestry_associations_file = "2024_APOE_ATN_biomarkers/Data/20250124_associations_by_local_genetic_ancestry.xlsx"
...
tbl5 <- data.frame("ATN biomarker" = c("Aꞵ42/40", "", "pTau-181", "", "NfL", "", "GFAP", ""), 
...
for(ancestry in sheet_names) { 
  ...
  ancestry_interaction_df <- ancestry_df %>% 
    ...
    select(c("est[CI]", "p-value"))
  tbl5 <- cbind(tbl5, ancestry_interaction_df, ancestry_allele_df)
} 
...
tbl5 <- rbind(row1, row2, row3, tbl5)

table5_file = "2024_APOE_ATN_biomarkers/Results/20250124_table5.xlsx"
write.xlsx(tbl5, file = table5_file, overwrite = TRUE, colNames=FALSE)  
