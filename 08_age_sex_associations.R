# Creating df for APOE-ATN associations, stratified by Age or Sex
...
APOE_ATN_interaction_by_AGE <- function(E2_exposure, E4_exposure, modelFormula, num_permutations) { 
  all_tables <- list()
  for (subset_group in ages) { 
    print(subset_group)
    svy_obj_subset <- subset(svy_obj, AGE_CAT == subset_group)
    E2 <- vector(mode = "list")
    E4 <- vector(mode = "list")
    for (var in outcome_vars){
      ...
      for (i in 1:num_permutations) {
        permuted_dat <- dat %>%
          mutate(across(all_of(outcome_vars), ~ ifelse(AGE_CAT == subset_group, sample(.), .))) 
        ...
        permuted_svy_obj_subset <- subset(permuted_svy_obj, AGE_CAT == subset_group)
...
ages <- unique(dat$AGE_CAT)

# Additive inheritance mode, stratified by AGE
e2_exposure = "E2_add"
e4_exposure = "E4_add"
model_formula = "~E2_add+E4_add+SEX+CENTER"
add_tbl_by_age <- APOE_ATN_interaction_by_AGE(e2_exposure, e4_exposure, model_formula, 10000)
association_file = "2024_APOE_ATN_biomarkers/Data/20250213_age_stratified_add_table.xlsx" 
# Write to xlsx sheet with a page per age group
write.xlsx(add_tbl_by_age, file = association_file, overwrite = TRUE) 

# Dominant inheritance mode, stratified by AGE
e2_exposure = "E2_dom"
e4_exposure = "E4_dom"
model_formula = "~E2_dom+E4_dom+SEX+CENTER"
dom_tbl_by_age <- APOE_ATN_interaction_by_AGE(e2_exposure, e4_exposure, model_formula, 10000)
association_file = "2024_APOE_ATN_biomarkers/Data/20250213_age_stratified_dom_table.xlsx" 
# Write to xlsx sheet with a page per age group
write.xlsx(dom_tbl_by_age, file = association_file, overwrite = TRUE) 


APOE_ATN_interaction_by_SEX <- function(E2_exposure, E4_exposure, modelFormula, num_permutations) { 
  all_tables <- list()
  for (subset_group in sexes) { 
    print(subset_group)
    svy_obj_subset <- subset(svy_obj, SEX == subset_group)
    E2 <- vector(mode = "list")
    E4 <- vector(mode = "list")
    for (var in outcome_vars){
      print(var)
      ...
      for (i in 1:num_permutations) {
        permuted_dat <- dat %>%
          mutate(across(all_of(outcome_vars), ~ ifelse(SEX == subset_group, sample(.), .))) 
        
        # Fit the model with permuted data
        permuted_svy_obj <- svydesign(id = ~PSU_ID, strata = ~STRAT, weights = ~WEIGHT_NORM_OVERALL_INCA, 
                                      nest = TRUE, data = permuted_dat)
        permuted_svy_obj_subset <- subset(permuted_svy_obj,SEX == subset_group)
        permuted_mod <- svyglm(formula = as.formula(paste0(var, modelFormula)), design = permuted_svy_obj_subset, family = gaussian())
...
e2_exposure = "E2_add"
e4_exposure = "E4_add"
model_formula = "~E2_add+E4_add+AGE+CENTER"
add_tbl_by_sex <- APOE_ATN_interaction_by_SEX(e2_exposure, e4_exposure, model_formula, 10000)
association_file = "2024_APOE_ATN_biomarkers/Data/20250224_sex_stratified_add_table.xlsx" 
# Write to xlsx sheet with a page per age group
write.xlsx(add_tbl_by_sex, file = association_file, overwrite = TRUE) 

# Dominant inheritance mode, stratified by AGE
e2_exposure = "E2_dom"
e4_exposure = "E4_dom"
model_formula = "~E2_dom+E4_dom+AGE+CENTER"
dom_tbl_by_sex <- APOE_ATN_interaction_by_SEX(e2_exposure, e4_exposure, model_formula, 10000)
association_file = "2024_APOE_ATN_biomarkers/Data/20250224_sex_stratified_dom_table.xlsx" 
# Write to xlsx sheet with a page per age group
write.xlsx(dom_tbl_by_sex, file = association_file, overwrite = TRUE) 
...
e2_exposure = "E2_add"
e4_exposure = "E4_add"
model_formula = "~E2_add+E4_add+CENTER"
add_tbl_by_age_sex <- APOE_ATN_interaction_by_AGE_SEX(e2_exposure, e4_exposure, model_formula, 10000)
association_file = "2024_APOE_ATN_biomarkers/Data/20250219_age_sex_stratified_add_table.xlsx" 
# Write to xlsx sheet with a page per age group
write.xlsx(add_tbl_by_age_sex, file = association_file, overwrite = TRUE) 
