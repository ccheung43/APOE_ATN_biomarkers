# Creating df for Table s8: Associations between ATN biomarkers and APOE allele by local genetic ancestry (dominant)
...
# merge with dat file
dat_with_local_ancestry <- left_join(dat, local_ancestry, by = "SOL_ID") %>% 
  mutate(LOCAL_PROP_AFR = LOCAL_COUNT_AFR / 2, 
         LOCAL_PROP_AMER = LOCAL_COUNT_AMER / 2, 
         LOCAL_PROP_EUR = LOCAL_COUNT_EUR/ 2)
...
APOE_ATN_interaction_by_local_ancestry <- function(E2_exposure, E4_exposure, covarsFormula, num_permutations) { 
  all_tables <- list()
  for (ancestry_group in local_genetic_ancestry_groups) { 
    ...
    for (var in outcome_vars){
      ...
      for (i in 1:num_permutations) {
        permuted_dat <- dat_with_local_ancestry %>%
...
local_genetic_ancestry_groups <- c("LOCAL_PROP_AFR", "LOCAL_PROP_EUR", "LOCAL_PROP_AMER") 
...
tbl_by_local_ancestry <- APOE_ATN_interaction_by_local_ancestry(e2_exposure, e4_exposure, covarsFormula, 10000)
names(tbl_by_local_ancestry) = c("AFRICAN", "EUROPEAN", "AMERINDIAN")
# Write to xlsx sheet with page for each ancestry group
local_ancestry_associations_file = "2024_APOE_ATN_biomarkers/Data/20250402_dominant_associations_by_local_genetic_ancestry.xlsx"
write.xlsx(tbl_by_local_ancestry, file = local_ancestry_associations_file, overwrite = TRUE) 
...
dat_file <- local_ancestry_associations_file
...
tbls8 <- data.frame("ATN biomarker" = c("Aꞵ42/40", "", "pTau-181", "", "NfL", "", "GFAP", ""), 
                   "APOE allele" = c("ε2", "ε4", "ε2", "ε4", "ε2", "ε4", "ε2", "ε4"))
for(ancestry in sheet_names) { 
  ...
  ancestry_interaction_df <- ancestry_df %>% 
    ...
    select(c("est[CI]", "p-value"))
  tbls8 <- cbind(tbls8, ancestry_interaction_df, ancestry_allele_df)
} 
...
tbls8 <- rbind(row1, row2, row3, tbls8)

table_s8_file = "2024_APOE_ATN_biomarkers/Results/20250402_table_s8.xlsx"
write.xlsx(tbls8, file = table_s8_file, overwrite = TRUE, colNames=FALSE)  
