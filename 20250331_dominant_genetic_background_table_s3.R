# Creating df for Table S3: Associations between ATN biomarkers and APOE allele by genetic analysis group (dominant mode)
...
table_s3_file = "2024_APOE_ATN_biomarkers/Results/20250331_table_s3.xlsx"
gengroup_associations_file = "2024_APOE_ATN_biomarkers/Data/20250331_dominant_associations_by_gengroup.xlsx"
...
e2_exposure = "E2_dom"
e4_exposure = "E4_dom"
model_formula = "~E2_dom+E4_dom+SEX+AGE+CENTER+EV1+EV2+EV3+EV4+EV5"
dom_tbl_by_gengroup <- APOE_ATN_interaction_by_GENGROUP(e2_exposure, e4_exposure, model_formula, 10000)

# Write to xlsx sheet with a page per gengroup
write.xlsx(dom_tbl_by_gengroup, file = gengroup_associations_file, overwrite = TRUE) 
...
tbls3 <- data.frame("ATN biomarker" = c("Aꞵ42/40", "", "pTau-181", "", "NfL", "", "GFAP", ""), 
...
tbls3 <- rbind(row1, row2, tbls3)

write.xlsx(tbls3, file = table_s3_file, overwrite = TRUE, colNames=FALSE)  
